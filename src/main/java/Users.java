...
public class Users {
    ...
    private static void writeToJson(File fileJSON, List<User> people) {
    ...
    private static void readFile(File fileGSON, List<User> people) {
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(fileGSON))) {
            ...
            while (dataFile != null) {
                ...
                if (!column[0].equals("name") && !column[1].equals("age")) {
                    people.add(new User(column[0], Integer.parseInt(column[1])));
                }
